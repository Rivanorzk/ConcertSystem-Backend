...
// Ambil event draft yang sales_start_at-nya sudah lewat/sekarang
export async function findDraftsReadyToPublish() {
    const [rows] = await db.query(
        `
        SELECT id, title
        FROM events
        WHERE status = 'draft'
          AND sales_start_at IS NOT NULL
          AND sales_start_at <= NOW()
        `
    );
    return rows;
}
